                   ...
                   struct boot_status *bs)
{
    ...
    bool header_is_valid = boot_is_header_valid(hdr, fap);
    BOOT_LOG_INF("Image is %s", header_is_valid ? "valid" : "invalid");

    if (!header_is_valid || FIH_NOT_EQ(fih_rc, FIH_SUCCESS)) {
